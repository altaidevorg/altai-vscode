# Remaining work plan — pure chrome through A6.213
...
Pure dual-chrome extraction continues (**A6.140–A6.213**: session history snippet + task assignment status chrome). This document sequences **everything still required**
...
| Track | State |
|---|---|
| Pure chrome `agent-ui` libs | ~A6.140–A6.213 on `altai-app` main (through task assignment status from live run) |

# Remaining work plan — pure chrome through A6.228
...
Pure dual-chrome extraction continues (**A6.140–A6.228**: catalog id lookup, task run outcome counts). This document sequences **everything still required**
...
| Track | State |
|---|---|
| Pure chrome `agent-ui` libs | ~A6.140–A6.228 on `altai-app` main (through task run outcome counts) |

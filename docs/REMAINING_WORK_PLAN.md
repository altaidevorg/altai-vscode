# Remaining work plan — pure chrome through A6.216
...
Pure dual-chrome extraction continues (**A6.140–A6.216**: notification inbox filter + session title map chrome). This document sequences **everything still required**
...
| Track | State |
|---|---|
| Pure chrome `agent-ui` libs | ~A6.140–A6.216 on `altai-app` main (through session id/title map chrome) |

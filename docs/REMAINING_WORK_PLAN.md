# Remaining work plan — pure chrome through A6.211
...
Pure dual-chrome extraction continues (**A6.140–A6.211**: inbox/plan queue/store helpers, automation list/filter/schedule + form error chrome). This document sequences **everything still required**
...
| Track | State |
|---|---|
| Pure chrome `agent-ui` libs | ~A6.140–A6.211 on `altai-app` main (through inbox view, plan queue, error/pending helpers, automation list) |

# Remaining work plan — pure chrome through A6.215
...
Pure dual-chrome extraction continues (**A6.140–A6.215**: task runs + notification inbox filter chrome). This document sequences **everything still required**
...
| Track | State |
|---|---|
| Pure chrome `agent-ui` libs | ~A6.140–A6.215 on `altai-app` main (through notification inbox filter/search chrome) |
